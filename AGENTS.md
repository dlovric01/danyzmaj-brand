...
- **Accessibility is deliberate**: the `<h1>` lives in `#plate`, permanently visible on BOTH pages (ink and paper) — the two themes must stay element-for-element identical, differing only in the swapped `--ink`/`--bone` values; the `svg` is `aria-hidden`, `#fx`/`#lure` are `aria-hidden`, and a `prefers-reduced-motion` query collapses all animations to 1ms so a scroll snaps straight to the burned/unburned end state — preserve all of these when editing.
...
- **Animation**: the rampage is a `wake` keyframe on `html.rampage` scrubbed by JS (`currentTime` driven from scroll); idle life runs on its own clock — `pulse` (`.acc`), `breath` (`.trigger`), `blink` (`.eye`), `lure` (`#lure`). **Any animation that must not be scrubbed by the rampage timeline MUST be added to the exclusion regex in the script** (`/^(pulse|blink|breath|lure)$/`), or the scroll will hijack its clock.
...
## Testing & QA
- No test framework. QA is visual: open the page, verify the mark at rest matches the original monogram exactly (plus idle life: breath, an eye blink every ~6.4s, the ember lure drifting at the bottom, the `danyzmaj` wordmark beneath), scroll and watch the full rampage (Z reassembles into the dragon → liftoff → charge → the page burns to paper), then scroll again to burn back. After each settle, diff the two pages mentally: same elements, same positions, only the ink/bone colors swapped — and the eye/mouth holes must blend seamlessly into the background in both.
- Also verify: keyboard (ArrowDown/PageDown drive the timeline, ArrowUp/PageUp retreat), stopping mid-scroll (the untended fire dies back down and disarms cleanly), no-JS (static Z), reduced-motion (one scroll tick toggles instantly, name legible in final ink), and mobile/desktop centering.
